...
Future<void> main(List<String> arguments) async {
  ...
  final fixtureFile = File(
    arguments.isNotEmpty
        ? arguments.first
        : '${scriptDirectory.path}/v3-js-decoder-fixtures.json',
  );
  final document =
  ...
  for (final raw in fixtures) {
    ...
    final decodedBody = codec.decodeBody(body);
    final decodedApp = codec.decodeAppPayloadWithoutSender(appPayload);
    final decodedText = codec.decodeText(fixture['text'] as String);
    for (final entry in <String, MCOImage>{
      'body': decodedBody,
      'app': decodedApp,
      'text': decodedText,
    }.entries) {
