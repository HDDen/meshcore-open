...
enum MCOImageV3BlockAlgorithm {
  ...
  // Alternate 7-bit continuation varuint bodies. These keep compactRle and
  // compactSparse untouched while restoring the value ranges where an
  // eight-bit varuint is shorter than the bounded compact integer grammar.
  varUintRle,
  varUintSparse;
...
class MCOImageV3Codec {
  ...
  /// for regular [encode] calls.
  ///
  /// In VS Code / Flutter, launch with:
  /// `--dart-define=MCOIMG_V3_CANDIDATE_DEBUG=true`
  ///
  /// [debugEncode] always collects diagnostics and emits the same log,
  /// independently of this compile-time flag.
  static const bool candidateDebugLoggingEnabled = bool.fromEnvironment(
    'MCOIMG_V3_CANDIDATE_DEBUG',
    defaultValue: false,
  );
  ...
  static const int _normalMaxRegions = 12;
  ...
  static const int _maxNormalBackgroundCandidates = 4;
  ...
  // Normal keeps the strong, relatively inexpensive region-body families.
  // It still skips direct codecs and full palette-order optimization;
  // High/Extreme retain the complete candidate table above.
  ...
  static const List<MCOImageV3BlockAlgorithm> _normalBlockAlgorithms = [
    MCOImageV3BlockAlgorithm.rawGlobal,
    MCOImageV3BlockAlgorithm.rawLocal,
    MCOImageV3BlockAlgorithm.compactRle,
    MCOImageV3BlockAlgorithm.varUintRle,
  ...
  static const List<MCOImageV3BlockAlgorithm> _fullBlockAlgorithms = [
    MCOImageV3BlockAlgorithm.rawGlobal,
    MCOImageV3BlockAlgorithm.rawLocal,
    MCOImageV3BlockAlgorithm.compactRle,
    MCOImageV3BlockAlgorithm.varUintRle,
  ...
  // Normal evaluates every non-hybrid compact Regions combination. This
  // recovers the largest wire-size wins (compact/delta/common/shared) while
  // leaving hybrid headers and beam geometry to High/Extreme.
  static const _normalRegionCandidateOptions = [
    (
  ...
  static const _fullRegionCandidateOptions = [
    ..._normalRegionCandidateOptions,
    (
      commonBlockHeader: true,
  ...
  static List<ScanMode> _regionScanModesForLevel({
    required bool useHighCompressionExtras,
    bool reducedCostEvaluator = false,
  }) {
    if (useHighCompressionExtras || reducedCostEvaluator) {
      return ScanMode.values;
    }
    return const <ScanMode>[ScanMode.h, ScanMode.v, ScanMode.s];
  }
  ...
  MCOImageEncodeDiagnostics _runEncode(
    MCOImage image, {
    int? backgroundColor,
    List<MCOImageBackgroundCandidate>? backgroundCandidates,
    List<ScanMode>? scanModes,
    required bool includeNonScanCandidates,
  ...
  MCOImageEncodeDiagnostics _encodeCandidates(
    ...
    for (final background in effectiveBackgroundCandidates) {
      ...
      if (includeNonScanCandidates) {
        ...
        for (final variant in regionVariants) {
          final regionPlanCache =
              variant.regionPlanCache ??
          ...
          if (!collectDiagnostics) {
            final bestOptions = _bestRegionCandidateOptions(
              image,
              bg,
              regionPlanCache,
              useHighCompressionExtras: useHighCompressionExtras,
            ...
            if (bestOptions != null) {
              addCandidate(
                _tryBuildRegionsCandidate(
                  ...
                  compactGeometry: bestOptions.compactGeometry,
                  commonBlockHeader: bestOptions.commonBlockHeader,
                  hybridCommonHeader: bestOptions.hybridCommonHeader,
                  deltaGeometry: bestOptions.deltaGeometry,
                  sharedLocalPalette: bestOptions.sharedLocalPalette,
                  useHighCompressionExtras: useHighCompressionExtras,
                ),
              );
            }
          ...
          final regionOptions = useHighCompressionExtras
              ? _fullRegionCandidateOptions
              : _normalRegionCandidateOptions;
      ...
      for (final scan in effectiveScanModes) {
        ...
        for (final algorithm in effectiveBlockAlgorithms) {
    ...
    for (final scan in effectiveScanModes) {
      ...
      for (final background in effectiveBackgroundCandidates) {
        final bg = background.color;
        for (final algorithm in topLevelBackgroundSensitiveAlgorithms) {
          if (_canUseCompactBlockHeader(algorithm)) {
            if (scan != ScanMode.h) continue;
            final compactCandidate = _tryBuildCandidate(
              image,
    ...
    if (candidatesByPayload == null) {
      return MCOImageEncodeDiagnostics(
        result: best,
        candidates: List<EncodedMCOImage>.unmodifiable([best]),
  ...
  ({
    ...
    bool commonBlockHeader,
    bool hybridCommonHeader,
    bool deltaGeometry,
    ...
    void consider({
      ...
      final plan = _regionPlanForCandidate(
        image,
        backgroundColor,
        regionPlanCache,
      ...
      final bitLength = _regionCandidateBitLength(
        image,
        backgroundColor,
        plan,
      ...
      if (best == null || byteLength < best!.byteLength) {
        best = (
          compactGeometry: compactGeometry,
          commonBlockHeader: commonBlockHeader,
          hybridCommonHeader: hybridCommonHeader,
    ...
    final optionsList = useHighCompressionExtras
        ? _fullRegionCandidateOptions
        : _normalRegionCandidateOptions;
  ...
  EncodedMCOImage? _tryBuildRegionsCandidate(
    ...
    assert(
      effectivePlanCache.backgroundColor == backgroundColor,
      'Region plan cache background mismatch',
    );
    assert(
    ...
    return _V3PreparedRegion(
      ...
      scanOrders: [
        for (final scan in ScanMode.values)
          useHighCompressionExtras || scan != ScanMode.sv
  ...
  static _V3RegionPlanCache _createRegionPlanCache(
    ...
    return _V3RegionPlanCache(
      preparedRegions: _prepareRegions(
        image,
        regions,
        useHighCompressionExtras: useHighCompressionExtras,
      ),
  ...
  List<List<int>> _sharedPaletteVariantsForActivePlan(
    MCOImage image, {
    required bool indexOrderSensitive,
  }) {
    final cache = _activeRegionPlanCache;
    if (cache == null) {
      throw const MCOImageInvalidInputException(
        'Shared palette generation requires an active region plan cache',
      );
    }
    final allPixels = cache.allRegionPixels ??= [
      for (final region in cache.preparedRegions) ...region.cropped,
    ];
    ...
    return cache.orderSensitiveSharedPaletteVariants ??=
      ...
      includeTransitionOrder:
          cache.useHighCompressionExtras && !cache.reducedCostEvaluator,
      includeBitplaneOptimizedOrder:
          cache.useHighCompressionExtras && !cache.reducedCostEvaluator,
      includeRgbOrder:
          cache.useHighCompressionExtras && !cache.reducedCostEvaluator,
      preferredFirstColor: cache.backgroundColor,
    );
  }

  int _sharedPaletteBitLengthForActivePlan(
    PaletteProfile profile,
    List<int> palette,
  ) {
    final cache = _activeRegionPlanCache;
    ...
    return cache.sharedPaletteBitLengthCache.putIfAbsent(key, () {
      final writer = _V3BitWriter();
      _writeLocalPalette(writer, profile, palette);
      return writer.bitLength;
    });
  ...
  _V3RegionPlan _regionPlanForCandidate(
    ...
    final previousCache = _activeRegionPlanCache;
    _activeRegionPlanCache = cache;
    try {
      return _regionPlanForCandidateWithActiveCache(
        image,
        backgroundColor,
        cache,
        commonBlockHeader: commonBlockHeader,
      ...
      _activeRegionPlanCache = previousCache;
    }
  }

  _V3RegionPlan _regionPlanForCandidateWithActiveCache(
    MCOImage image,
    int backgroundColor,
    _V3RegionPlanCache cache, {
    required bool commonBlockHeader,
  ...
  List<_V3RegionBlock>? _bestCommonRegionBlocks(
    ...
    for (final scan in _regionScanModesForLevel(
      useHighCompressionExtras: useHighCompressionExtras,
      reducedCostEvaluator: reducedCostEvaluator,
    )) {
    ...
    for (final scan in _regionScanModesForLevel(
      ...
      for (final algorithm in _sharedPaletteRegionAlgorithmsForLevel(
        ...
        for (final palette in paletteVariants) {
          var totalBits =
              _sharedPaletteBitLengthForActivePlan(
                image.paletteProfile,
                palette,
              ) +
  ...
  _V3RegionPlan _bestIndividualSharedLocalPaletteRegionPlan(
    ...
    final paletteVariants = _sharedPaletteVariantsForActivePlan(
      image,
      indexOrderSensitive: true,
    );
    ...
    int? bestBits;
    for (final palette in paletteVariants) {
      var totalBits = _sharedPaletteBitLengthForActivePlan(
        image.paletteProfile,
        palette,
      );
      final blocks = <_V3RegionBlock>[];
  ...
  _V3RegionPlan _bestHybridSharedLocalPaletteRegionPlan(
    ...
    int? bestBits;
    for (final palette in paletteVariants) {
      final paletteBitLength = _sharedPaletteBitLengthForActivePlan(
        image.paletteProfile,
        palette,
      );
  ...
  _V3RegionBlock? _tryBuildRegionBlock(
    ...
    final cache = _activeRegionPlanCache;
    final regionIndex = cache?.regionIndices[region];
    if (cache != null &&
        regionIndex != null &&
      ...
      cache.regionBlockCache[key] = block;
      return block;
    }
    return _buildRegionBlock(
      image,
      region,
      backgroundColor,
      algorithm,
  ...
  _V3RegionBlock? _buildRegionBlock(
    MCOImage image,
    _V3PreparedRegion region,
    int backgroundColor,
    MCOImageV3BlockAlgorithm algorithm,
    ...
    try {
      _writeBlockBody(
        ...
        greedyLzOnly: greedyLzOnly,
        useHighCompressionExtras:
            _activeRegionPlanCache?.useHighCompressionExtras ?? true,
      );
    } on MCOImageCodecException {
      return null;
    }
  ...
  _V3RegionBlock? _tryBuildSharedPaletteRegionBlock(
    ...
    if (cache != null &&
      ...
      final key = (
        ...
        scan.index,
        greedyLzOnly,
        String.fromCharCodes(palette),
      );
      if (cache.sharedRegionBlockCache.containsKey(key)) {
        return cache.sharedRegionBlockCache[key];
      }
      final block = _buildSharedPaletteRegionBlock(
        image,
        region,
        backgroundColor,
        algorithm,
      ...
      cache.sharedRegionBlockCache[key] = block;
      return block;
    }
    return _buildSharedPaletteRegionBlock(
      image,
      region,
      backgroundColor,
      algorithm,
  ...
  _V3RegionBlock? _buildSharedPaletteRegionBlock(
    MCOImage image,
    _V3PreparedRegion region,
    int backgroundColor,
    MCOImageV3BlockAlgorithm algorithm,
  ...
  _V3BlockBodyEncoding? _tryBuildBlockBodyEncoding(
    ...
    bool greedyLzOnly = false,
    bool useHighCompressionExtras = true,
  }) {
    ...
    switch (algorithm) {
      ...
      case MCOImageV3BlockAlgorithm.compactRle:
        _writeBestLocalPaletteBlock(
          ...
          writeBody: (bodyWriter, localPixels, bits) {
            var consumed = 0;
            for (final run in _buildRunsCached(localPixels)) {
              bodyWriter
      ...
      case MCOImageV3BlockAlgorithm.varUintRle:
        _writeBestLocalPaletteBlock(
          ...
          includeTransitionOrder: useHighCompressionExtras,
          writeBody: (bodyWriter, localPixels, bits) {
            // Pixel count terminates the stream, so unlike v2 there is no
            // redundant run-count field.
            for (final run in _buildRunsCached(localPixels)) {
              bodyWriter
                ..writeBits(run.color, bits)
      ...
      case MCOImageV3BlockAlgorithm.adaptiveBitplanes:
        _writeBestLocalPaletteBlock(
          ...
          indexOrderSensitive: true,
          includeTransitionOrder: useHighCompressionExtras,
          includeBitplaneOptimizedOrder: useHighCompressionExtras,
          includeRgbOrder: useHighCompressionExtras,
          preferredFirstColor: backgroundColor,
      ...
      case MCOImageV3BlockAlgorithm.varUintSparse:
        ...
        final sparseAnalysis = _sparseAnalysis(linear, backgroundColor);
        final segments = sparseAnalysis.segments;
        if (segments.isEmpty) {
          throw const MCOImageInvalidInputException(
            'Empty varuint sparse body',
          );
        }
        final nonBg = sparseAnalysis.foregroundColors;
        if (nonBg.isEmpty) {
          throw const MCOImageInvalidInputException(
            'Empty varuint sparse palette',
          );
        }
  ...
  void _writeBlockBodyWithSharedPalette(
    ...
    if (linear.isEmpty) {
      throw const MCOImageInvalidInputException(
        'Shared-palette block has no pixels',
      );
    }
    final map = _localIndexMapCached(palette);
    final bits = _localBits(palette.length);
    List<int> localPixels() => _localPixelsForPalette(linear, palette);
    ...
    switch (algorithm) {
      case MCOImageV3BlockAlgorithm.rawLocal:
        ...
        break;
      case MCOImageV3BlockAlgorithm.varUintRle:
        for (final run in _buildRunsCached(linear)) {
          writer
      ...
      case MCOImageV3BlockAlgorithm.varUintSparse:
        if (linear.every((pixel) => pixel == backgroundColor)) {
          throw const MCOImageInvalidInputException(
            'Shared varuint sparse has no foreground pixels',
          );
        }
        final segments = _sparseAnalysis(linear, backgroundColor).segments;
        if (segments.isEmpty) {
          throw const MCOImageInvalidInputException(
            'Shared varuint sparse has no segments',
          );
        }
        writer.writeBitVarUint(segments.length);
        var pos = 0;
        for (final segment in segments) {
          ...
          if (colorIndex == null) {
            throw const MCOImageInvalidInputException(
              'Shared varuint sparse color is missing from palette',
            );
          }
          writer
            ..writeBitVarUint(segment.start - pos)
    ...
    switch (algorithm) {
      ...
      case MCOImageV3BlockAlgorithm.varUintSparse:
        ...
        if (palette.contains(background)) {
          throw const MCOImageInvalidPayloadException(
            'Varuint sparse palette contains background',
          );
        }
        final bits = _localBits(palette.length);
        ...
        if (segmentCount <= 0 || segmentCount > count) {
          throw const MCOImageInvalidPayloadException(
            'Invalid varuint sparse segment count',
          );
        }
        var pos = 0;
        for (var i = 0; i < segmentCount; i++) {
          ...
          if (skip > count - pos) {
            throw const MCOImageInvalidPayloadException(
              'Invalid varuint sparse skip',
            );
          }
          pos += skip;
          if (pos >= count) {
            throw const MCOImageInvalidPayloadException(
              'Invalid varuint sparse segment start',
            );
          }
          final colorIndex = reader.readBits(bits);
          if (colorIndex >= palette.length) {
            throw const MCOImageInvalidPayloadException(
              'Varuint sparse color index out of range',
            );
          }
          final length = reader.readBitVarUint();
          if (length <= 0 || pos + length > count) {
            throw const MCOImageInvalidPayloadException(
              'Invalid varuint sparse segment length',
            );
          }
    ...
    switch (algorithm) {
      ...
      case MCOImageV3BlockAlgorithm.varUintRle:
        ...
        while (result.length < count) {
          ...
          if (colorIndex >= palette.length) {
            throw const MCOImageInvalidPayloadException(
              'Varuint RLE shared palette index out of range',
            );
          }
          final length = reader.readBitVarUint();
          if (length <= 0 || result.length + length > count) {
            throw const MCOImageInvalidPayloadException(
              'Invalid shared varuint RLE length',
            );
          }
          result.addAll(List<int>.filled(length, palette[colorIndex]));
        }
      ...
      case MCOImageV3BlockAlgorithm.varUintSparse:
        ...
        if (segmentCount <= 0 || segmentCount > count) {
          throw const MCOImageInvalidPayloadException(
            'Invalid shared varuint sparse segment count',
          );
        }
        ...
        for (var i = 0; i < segmentCount; i++) {
          ...
          if (skip > count - pos) {
            throw const MCOImageInvalidPayloadException(
              'Invalid shared varuint sparse skip',
            );
          }
          pos += skip;
          if (pos >= count) {
            throw const MCOImageInvalidPayloadException(
              'Invalid shared varuint sparse segment start',
            );
          }
          final colorIndex = reader.readBits(bits);
          if (colorIndex >= palette.length ||
              palette[colorIndex] == backgroundColor) {
            throw const MCOImageInvalidPayloadException(
              'Invalid shared varuint sparse color',
            );
          }
          final length = reader.readBitVarUint();
          if (length <= 0 || pos + length > count) {
            throw const MCOImageInvalidPayloadException(
              'Invalid shared varuint sparse segment length',
            );
          }
          for (var j = 0; j < length; j++) {
            result[pos + j] = palette[colorIndex];
  ...
  static ImageMode _imageModeForAlgorithm(MCOImageV3BlockAlgorithm algorithm) {
    return switch (algorithm) {
      ...
      MCOImageV3BlockAlgorithm.compactRle ||
      MCOImageV3BlockAlgorithm.varUintRle => ImageMode.rleLocal,
      MCOImageV3BlockAlgorithm.compactSparse ||
      MCOImageV3BlockAlgorithm.varUintSparse => ImageMode.sparseBg,
  ...
  static String _blockAlgorithmLabel(MCOImageV3BlockAlgorithm algorithm) {
    return switch (algorithm) {
      ...
      MCOImageV3BlockAlgorithm.varUintRle => 'RLE varuint',
      MCOImageV3BlockAlgorithm.compactSparse => 'Sparse background',
      MCOImageV3BlockAlgorithm.varUintSparse => 'Sparse varuint',
  ...
  List<_V3Run> _buildRunsCached(List<int> pixels) {
  ...
  void _writeBestLocalPaletteBlock(
    _V3BitWriter writer,
    List<int> linear,
    PaletteProfile profile, {
    required bool indexOrderSensitive,
  ...
  _V3LocalPaletteEncoding _encodeLocalPaletteBlock(
    List<int> linear,
    PaletteProfile profile,
    List<int> palette,
    void Function(
  ...
  List<List<int>> _localPaletteVariants(
    List<int> pixels,
    PaletteProfile profile, {
    required bool indexOrderSensitive,
    required bool includeTransitionOrder,
    ...
    final cached = _localPaletteVariantCache[cacheKey];
    ...
    _localPaletteVariantCache[cacheKey] = variants;
    return variants;
  }
  ...
  static _V3AdaptiveBitplaneDecision _chooseAdaptiveBitplaneEncoding(
    ...
    if (pixels.isEmpty) {
      throw const MCOImageInvalidInputException(
        'Adaptive bitplanes require at least one pixel',
      );
    }
    final runs = _buildBitplaneRuns(pixels, bit);
  ...
  static List<_V3BackgroundCandidate> _normalBackgroundCandidates(
    MCOImage image,
    int preferredBackground,
  ) {
    final result = <_V3BackgroundCandidate>[];
    final seen = <int>{};

    void add(int color, int rank) {
      if (result.length >= _maxNormalBackgroundCandidates) return;
  ...
  List<_V3RegionVariant> _regionVariantsForBackground(
    ...
    required bool useNormalSharedBeam,
  }) {
    ...
    final greedyVariants = _findGreedyRectRegionVariants(
      image.pixels,
      image.width,
      image.height,
      backgroundColor,
      maxRegions,
      firstOnly: !useHighCompressionExtras,
    );
    for (final regions in greedyVariants) {
        ...
        if (seen.add(key)) {
          result.add(
            _V3RegionVariant(
              state.regions,
              'beam',
              regionPlanCache: state.exactPlanCache,
            ),
          );
        }
      }
    ...
    if (useNormalSharedBeam &&
        result.isNotEmpty &&
        image.pixels.length <= _maxBeamRegionPixels) {
      final beamVariants = _findNormalSharedCompactRegionVariants(
        image,
        backgroundColor,
        result.map((variant) => variant.regions).toList(growable: false),
        beamMaxRegions,
  ...
  List<_V3RegionBeamState> _findPayloadOptimizedRegionVariants(
    ...
    for (final state in reducedResult) {
      final exactResultForLayout = _exactRegionPayloadCost(
        image,
        backgroundColor,
        state.regions,
        useHighCompressionExtras: useHighCompressionExtras,
      );
      if (exactResultForLayout != null) {
        exactResult.add(
          _V3RegionBeamState(
            state.regions,
            exactResultForLayout.cost,
            exactPlanCache: exactResultForLayout.planCache,
          ),
        );
      }
    }
  ...
  List<_V3RegionBeamState> _findNormalSharedCompactRegionVariants(
    MCOImage image,
    int backgroundColor,
    List<List<_V3Bounds>> initialVariants,
    int maxRegions,
    ...
    for (final regions in initialVariants) {
      ...
      final key = _regionListKey(normalized);
      if (!seen.add(key)) continue;
      final exactCost = _normalSharedCompactRegionPayloadCost(
        image,
        backgroundColor,
        normalized,
      );
      if (exactCost != null) {
        initialStates.add(
          _V3RegionBeamState(
            normalized,
            exactCost.cost,
            exactPlanCache: exactCost.planCache,
          ),
        );
      }
    }
    ...
    for (var depth = 0; depth < _regionBeamDepth; depth++) {
      ...
      for (final state in beam) {
        for (final regions in _regionBeamNeighborsFor(
          ...
          if (exactCost == null) continue;
          final candidate = _V3RegionBeamState(
            regions,
            exactCost.cost,
            exactPlanCache: exactCost.planCache,
          );
          next.add(candidate);
          if (candidate.cost < bestExistingCost) improved.add(candidate);
        }
      }
      if (next.isEmpty) break;
      next.sort((left, right) => left.cost.compareTo(right.cost));
    ...
    for (final state in improved) {
      if (resultKeys.add(_regionListKey(state.regions))) result.add(state);
      if (result.length >= _regionBeamWidth) break;
    }
    return result;
  }

  _V3ExactRegionCost? _normalSharedCompactRegionPayloadCost(
    MCOImage image,
    int backgroundColor,
    List<_V3Bounds> regions,
  ) {
    final regionPlanCache = _createRegionPlanCache(
      ...
      regions,
      backgroundColor: backgroundColor,
      useHighCompressionExtras: false,
      reducedCostEvaluator: false,
    );
    int? bestBitLength;
    for (final options in _normalRegionCandidateOptions) {
      ...
      if (bestBitLength == null || bitLength < bestBitLength) {
  ...
  int? _regionPayloadByteCost(
    ...
    return _exactRegionPayloadCost(
      image,
      backgroundColor,
      regions,
      useHighCompressionExtras: useHighCompressionExtras,
  ...
  _V3ExactRegionCost? _exactRegionPayloadCost(
    MCOImage image,
    int backgroundColor,
    List<_V3Bounds> regions, {
    required bool useHighCompressionExtras,
...
class _V3ExactRegionCost {
  final int cost;
  final _V3RegionPlanCache planCache;

  const _V3ExactRegionCost(this.cost, this.planCache);
}
...
class _V3RegionPlanCache {
  ...
  List<List<int>>? orderInvariantSharedPaletteVariants;
  List<List<int>>? orderSensitiveSharedPaletteVariants;
