...
### Scanning a channel QR (fork-only, removable)
...
Upstream may ship its own version, so the feature is kept in one piece that can be lifted out: the parser `helpers/channel_qr_link.dart` (pure Dart, tested in `test/helpers/channel_qr_link_test.dart`), the screen `screens/channel_qr_scanner_screen.dart` with its `scanAndAdd` helper, and four insertions in `channels_screen.dart` marked `channel-qr-scan` (two imports, option card `6`, its `case 6`). `ChannelQrLink.enabled` hides the option without touching anything else. To remove it, delete the two files and the test, drop the marked lines, and remove the ARB keys `channels_scanQrInstructions`, `channels_invalidQrCode` and `channels_qrAlreadyAdded`; `channels_scanQrCode` and `community_join` were there before and stay.
