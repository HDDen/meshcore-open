...
- header: 12 bits на сообщение (версия, поколение моделей, языки A и B); RAW_UTF8-кандидат
  считается с 16-битным заголовком и выбирается на уровне сообщения, когда он короче.

В отчёте отдельно показываются:

- token-only bits/char;
- bits/char с 12-bit header на каждое validation message.
...
Заголовок потока несёт номер поколения набора таблиц: поле `generation`, значения 0..6
коротко, 7 и выше через escape, максимум 262. Поколение относится ко всему набору сразу, не к
отдельному языку:

- перегенерация уже существующей таблицы меняет её wire-хэш и требует нового поколения;
- добавление таблиц зарезервированным языкам (DE, IT, UK, BE) поколение не меняет;
- изменение страницы пунктуации, дерева токенов, контекстов или ограничений таблиц это новая
  версия механизма; поколения нумеруются внутри версии, пара (версия, поколение) однозначно
  задаёт набор таблиц.

Текущее поколение записано в `model_manifest.json` как `modelGeneration` (сейчас `0`) и в
тренере как `MODEL_GENERATION`; `verify_runtime_models.py` проверяет, что реестр
`mcotxt_model_registry.dart` содержит набор с тем же номером. Декодер без таблиц нужного
поколения отвечает `unsupportedModelGeneration`; RAW_UTF8-сообщения читаются при любом поколении.
