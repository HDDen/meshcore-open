...
void main() {
  ...
  group('MCOImageV3Codec roundtrip', () {
    ...
    test(
      'sparse multicolor image roundtrips through compact sparse candidates',
      () {
  ...
  group('MCOImageV3Codec app payload', () {
    test('payload without sender is subtypeVersion followed by body', () {
      ...
      expect(Uint8List.sublistView(payload, 1), encoded.body);
    ...
    test(
      'channel binary helper decodes app data separately from legacy text',
      () {
        final image = _image(4, 4, (x, y) => (x + y) & 1);
        final encoded = codec.encode(image);
